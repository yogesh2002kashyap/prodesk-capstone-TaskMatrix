    ...
    return(
        <aside className="w-[188px] min-w-[188px] h-screen bg-white border-r border-gray-100 flex flex-col">
            ...
            <div className="flex flex-col flex-1 overflow-y-auto px-2 py-3 gap-1">
                ...
                {navItems.map((item) => (
                    <button key={item.path} onClick={() => navigate(item.path)} className={`w-full text-left px-3 py-2 rounded-md text-xs transition ${
                    location.pathname === item.path ? 'bg-gray-50 text-gray-900 font-medium' : 'text-gray-400 hover:bg-gray-50 hover:text-gray-800'}`} >{item.label}</button>
                ))
                }
